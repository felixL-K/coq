...
let cache_one_scheme kind (ind,const) =
  scheme_map := Indmap.update ind (function
      | None -> Some (CList.Map.singleton kind const)
      | Some map -> Some (CList.Map.add kind const map))
      !scheme_map
...
let lookup_scheme kind ind = CList.Map.find kind (Indmap.find ind !scheme_map)
