...
let bool2leibniz_scheme_msg ind = (* TODO: mutual inductive case *)
  str "Boolean to Leibniz on " ++ quote (Printer.pr_inductive (Global.env()) ind)

let leibniz2bool_scheme_msg ind = (* TODO: mutual inductive case *)
  str "Leibniz to Boolean on " ++ quote (Printer.pr_inductive (Global.env()) ind)
...
(* ======================================================================================== *)  
   ...
   let beq_scheme_kind =
     ...
     ~deps:(fun env mutind -> try_declare_scheme_dep (beq_scheme_msg (mutind,0)) build_beq_scheme_deps UserIndividualRequest env mutind)
     (fun env handle kn -> try_declare_scheme (beq_scheme_msg (List.hd kn)) build_beq_scheme UserIndividualRequest env handle kn)

   let beq_scheme_kind_internal =
     Ind_tables.declare_mutual_scheme_object (["Boolean";"Equality";"Internal"], Some InType, true)
     (fun id -> match id with None -> "beq" | Some i -> (Id.to_string i.mind_typename) ^ "_" ^ "beq")
     ~deps:(fun env mutind -> try_declare_scheme_dep (beq_scheme_msg (mutind,0)) build_beq_scheme_deps UserIndividualRequest env mutind)
     (fun env handle kn -> try_declare_scheme (beq_scheme_msg (List.hd kn)) build_beq_scheme UserAutomaticRequest env handle kn)
...
let make_bl_scheme env handle indl =
   ...
   let make_lb_scheme env handle indl =
     let (mind,i) = match indl with hd::r -> hd | _ -> assert false in
     let mib = Environ.lookup_mind mind env in
     if not (Int.equal (Array.length mib.mind_packets) 1) then
       CErrors.user_err
   ...
   let lb_scheme_kind =
     ...
     (fun env handle kn -> try_declare_scheme (leibniz2bool_scheme_msg (List.hd kn)) make_lb_scheme UserIndividualRequest env handle kn)

   let lb_scheme_kind_internal =
     Ind_tables.declare_mutual_scheme_object (["Leibniz";"Boolean";"Internal"], Some InType, true)
     (fun id -> match id with None -> "dec_lb" | Some i -> (Id.to_string i.mind_typename) ^ "_" ^ "dec_lb")
     ~deps:make_lb_scheme_deps
     (fun env handle kn -> try_declare_scheme (leibniz2bool_scheme_msg (List.hd kn)) make_lb_scheme UserAutomaticRequest env handle kn)
...
(* let try_make_eq_decidability env handle indl = *)
(*   try_declare_scheme  *)
   ...
   let eq_dec_scheme_kind_internal =
     ...
     (fun env handle kn -> try_declare_scheme (eq_dec_scheme_msg (List.hd kn)) make_eq_decidability UserAutomaticRequest env handle kn)
