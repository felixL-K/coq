...
- [ ] Deplacer dans la fonction de Case erreur dans elimscheme.

- [ ] internal dans LES beq : à voir pk car on le def dans la fct eq_dec de auto_ind_decl normalement
...
- [ ] la fonction suff : rajouter un argument Sorts.family mais qui n est pas celui d'arriver, mais de depart : cad le sort du type de l'inductif sur lequel on fait le schema
